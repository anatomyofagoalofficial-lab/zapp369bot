    ...
    with _conn() as c:
        c.executescript(
            ...
            CREATE TABLE IF NOT EXISTS settings (
                ...
                personality_on INTEGER DEFAULT 0,
        ...
        for stmt in (
            ...
            "ALTER TABLE settings ADD COLUMN personality_on INTEGER DEFAULT 0",
...
# ---------------------------------------------------------------------------
# Bot self-throttle — hard safety net
# Even if a bug tries to make the bot spam, the throttle silently drops
# outgoing messages once the limit is hit. Per chat, per rolling window.
# ---------------------------------------------------------------------------
BOT_THROTTLE_LIMIT = 10        # max messages
BOT_THROTTLE_WINDOW = 30       # per this many seconds
_bot_send_tracker = defaultdict(lambda: deque(maxlen=64))  # chat_id -> deque[ts]


def _bot_throttle_ok(chat_id) -> bool:
    """Return False if the bot has already sent BOT_THROTTLE_LIMIT msgs in the window."""
    if chat_id is None:
        return True
    dq = _bot_send_tracker[chat_id]
    now = time.time()
    # Drop old timestamps
    while dq and now - dq[0] > BOT_THROTTLE_WINDOW:
        dq.popleft()
    if len(dq) >= BOT_THROTTLE_LIMIT:
        # Log once per overflow (not every call) so the deploy logs aren't spammed
        if not dq or now - dq[-1] > 1:
            logger.warning("Bot throttle hit in chat %s (%d msgs / %ds) — silencing",
                           chat_id, len(dq), BOT_THROTTLE_WINDOW)
        return False
    dq.append(now)
    return True
...
ADMIN_HELP_TEXT = (
    ...
    "/autoleaderboard on|off|here|test — Sunday 9pm Top 10\n"
    "/personality on|off — in-character replies (off by default)\n"
...
async def flood_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    detail = "off" if not n else f"{n} msgs/10s → {action}"
    await update.effective_message.reply_text(f"🌊 Anti-flood: {detail}.")
...
def register_fun(app):
    ...
    app.add_handler(CommandHandler(["personality", "zappmode"], personality_cmd))
    app.add_handler(CommandHandler(["antispam", "antiflood"], antispam_cmd))
...
async def _personality_try_reply(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    # SAFETY GATE: personality is OFF by default per chat. Admin must enable
    # via /personality on. This prevents accidental spam on existing groups.
    try:
        s = db.get_settings(chat_id)
        if not (s["personality_on"] if "personality_on" in s.keys() else 0):
            return False
    except Exception:  # noqa: BLE001
        return False
    ...
    # STRICT MODE: personality ONLY fires on @-mention or direct reply-to-bot.
    # No more bare-keyword triggers (gm/gn, "wen moon", etc.) — those were
    # too noisy and felt spammy. Users who want a chat ping the bot by name.
    if not (mentioned_bot or replied_to_bot):
        return False
    ...
    # Hard safety: respect the bot self-throttle even if all other checks pass.
    if not _bot_throttle_ok(chat_id):
        return False
...
async def antispam_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show + configure flood protection (user-side limit + bot self-throttle)."""
    chat_id = target_chat(update)
    msg = update.effective_message
    args = [a for a in context.args]
    sub = args[0].lower() if args else "status"
    ...
    if sub in ("on", "enable"):
        # Sensible default: 10 user messages per 10s → mute on flood
        db.set_setting(chat_id, "flood_limit", 10)
        db.set_setting(chat_id, "flood_action", "mute")
        await msg.reply_text(
            "🌊 <b>Anti-spam ON</b> ⚡\n"
            f"• Users: max <b>10 msgs / {FLOOD_WINDOW}s</b> → 1h mute on flood\n"
            f"• Bot: hard cap <b>{BOT_THROTTLE_LIMIT} msgs / {BOT_THROTTLE_WINDOW}s</b> per chat\n\n"
            "Adjust user limit: /setflood 8 (msgs / 10s; 0 = off)\n"
            "Change action: /setfloodaction mute|kick|ban",
            parse_mode=ParseMode.HTML)
        return
    if sub in ("off", "disable"):
        db.set_setting(chat_id, "flood_limit", 0)
        await msg.reply_text(
            "🌊 User-side anti-spam <b>OFF</b>.\n"
            "<i>Bot self-throttle is always active and cannot be disabled.</i>",
            parse_mode=ParseMode.HTML)
        return

    # Status
    limit = s["flood_limit"]
    action = s["flood_action"] or "mute"
    user_state = f"max {limit} msgs / {FLOOD_WINDOW}s → {action}" if limit else "OFF"
    await msg.reply_text(
        "🌊 <b>Anti-spam status</b>\n\n"
        f"<b>Users:</b> {user_state}\n"
        f"<b>Bot self-throttle:</b> {BOT_THROTTLE_LIMIT} msgs / {BOT_THROTTLE_WINDOW}s (always on)\n\n"
        "Commands:\n"
        "/antispam on — enable both layers\n"
        "/antispam off — disable user-side\n"
        "/setflood &lt;n&gt; — fine-tune user limit\n"
        "/setfloodaction mute|kick|ban — choose action",
        parse_mode=ParseMode.HTML)
...
async def personality_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Toggle the ZAPP personality replies on/off for this chat (off by default)."""
    chat_id = target_chat(update)
    msg = update.effective_message
    args = [a for a in context.args]
    sub = args[0].lower() if args else "status"
    s = db.get_settings(chat_id)
    on = s["personality_on"] if "personality_on" in s.keys() else 0
    if sub == "on":
        db.set_setting(chat_id, "personality_on", 1)
        await msg.reply_text(
            "🤖 <b>Personality ON</b> ⚡\n"
            "I'll reply in-character when @-mentioned or when someone replies to one of my messages.\n"
            "Use /personality off to turn it off any time.",
            parse_mode=ParseMode.HTML)
        return
    if sub == "off":
        db.set_setting(chat_id, "personality_on", 0)
        await msg.reply_text("🤖 Personality <b>OFF</b>. I'll only respond to commands.",
                             parse_mode=ParseMode.HTML)
        return
    state = "ON ✅" if on else "OFF"
    await msg.reply_text(
        f"🤖 <b>Personality:</b> {state}\n\n"
        "Off by default. When ON, I reply in-character only when:\n"
        "• Someone @-mentions me\n"
        "• Someone replies directly to one of my messages\n\n"
        "/personality on — enable\n"
        "/personality off — disable",
        parse_mode=ParseMode.HTML)
...
async def _post_init(app):
    ...
    try:
        await app.bot.set_my_commands([
            ...
            ("personality", "ZAPP chat replies on/off (admin)"),
            ("antispam", "Flood protection settings (admin)"),
