...
_recent_welcomes = {}          # (chat_id, user_id) -> ts  guards duplicate welcome messages
_WELCOME_TTL = 90              # seconds

def _already_welcomed(chat_id, user_id, now):
    """True if this member was welcomed moments ago. Prevents the double welcome
    caused by a re-delivered update or by a second bot instance sharing the process."""
    for k, t in list(_recent_welcomes.items()):
        if now - t > _WELCOME_TTL:
...
async def on_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for member in update.effective_message.new_chat_members:
        if member.id == context.bot.id:
            continue
        if _already_welcomed(chat.id, member.id, now):
            logger.info("skipping duplicate welcome for %s in %s", member.id, chat.id)
            continue
